...
SQLCommand = ("DELETE icomparev2.imp_promotion;")
...
while not ended:
    ...
    if planid == None and promotion_text == None and  startdate == None:
        ...
        if language == None:
            SQLCommand = u"INSERT INTO icomparev2.imp_promotion (plan_id, promotion, language, type, active, startdate, enddate) VALUES (" +str(planid)+",N'" + promotion_text+"','en','cp',"+str(active)+",'"+str(startdate)+"','"+str(enddate)+"');"
            cursor.execute(SQLCommand)
            SQLCommand = u"INSERT INTO icomparev2.imp_promotion (plan_id, promotion, language, type, active, startdate, enddate) VALUES (" +str(planid)+",N'" + promotion_text+"','zh','cp',"+str(active)+",'"+str(startdate)+"','"+str(enddate)+"');"
            cursor.execute(SQLCommand)
        else:
            SQLCommand = u"INSERT INTO icomparev2.imp_promotion (plan_id, promotion, language, type, active, startdate, enddate) VALUES (" +str(planid)+",N'" + promotion_text+"','"+language+"','cp',"+str(active)+",'"+str(startdate)+"','"+str(enddate)+"');"
            cursor.execute(SQLCommand)
...
#    enddate = ws.cell(row=r, column=7).value
    ...
    if planid == None and promotion_text == None and  startdate == None:
        ...
        SQLCommand = u"INSERT INTO icomparev2.imp_promotion (plan_id, promotion, language, type, active, startdate, enddate) VALUES (" +str(planid)+",N'" + promotion_text+"','en','bv',"+str(active)+",'"+str(startdate)+"','"+str(enddate)+"');"
        print SQLCommand
        cursor.execute(SQLCommand)
        SQLCommand = u"INSERT INTO icomparev2.imp_promotion (plan_id, promotion, language, type, active, startdate, enddate) VALUES (" +str(planid)+",N'" + promotion_text+"','zh','bv',"+str(active)+",'"+str(startdate)+"','"+str(enddate)+"');"
        print SQLCommand
        cursor.execute(SQLCommand)
...
SQLCommand = u"insert into icomparev2.convoy_feature  select plan_id, case when type='cp' then 'promotion' else 'bv_promotion' end as feature_name, stuff((select '<br><br>' +  (promotion) from icomparev2.imp_promotion where plan_id = a.plan_id and language = a.language and type = a.type and active = a.active order by type desc FOR XML PATH(''),type).value('(./text())[1]','nvarchar(max)'),1,8,'') as promotion, 'D' as display_type, 'text' as data_type, active, getdate(), language, min(startdate) as startdate , max(enddate) as enddate from icomparev2.imp_promotion a where getdate()>=startdate and getdate() <=enddate and active = 1 and not exists (select 1 from icomparev2.convoy_feature f where f.plan_id = a.plan_id and f.language = a.language and f.feature_name = case when a.type ='cp' then 'promotion' else 'bv_promotion' end ) group by plan_id, language,active, type"
print SQLCommand
cursor.execute(SQLCommand)

## inactivate promotions
SQLCommand = u"update icomparev2.convoy_feature set is_active = 0 , display_start_date = ip.startdate, display_end_date = ip.enddate from ( select * from icomparev2.imp_promotion where active = 0 or not (getdate() >= startdate and getdate() <= enddate)) ip where icomparev2.convoy_feature.plan_id = ip.plan_id  and icomparev2.convoy_feature.feature_name = case when ip.type = 'cp' then 'promotion' else 'bv_promotion' end"
print SQLCommand
cursor.execute(SQLCommand)

## update promotions
SQLCommand = u"update icomparev2.convoy_feature set feature_value = ip.promotion, display_start_date = ip.startdate, display_end_date = ip.enddate, is_active=1 from (select plan_id, type, stuff((select '<br><br>' +  (promotion) from icomparev2.imp_promotion   where plan_id = a.plan_id   and language = a.language   and active = a.active and type = a.type and getdate()>=startdate and getdate() <=enddate and active = 1 order by type desc FOR XML PATH(''),type).value('(./text())[1]','nvarchar(max)'),1,8,'') as promotion, language,  min(startdate) as startdate , max(enddate) as enddate from icomparev2.imp_promotion a where getdate()>=startdate and getdate() <=enddate and active = 1 group by plan_id, language,active, type ) ip where icomparev2.convoy_feature.plan_id = ip.plan_id and icomparev2.convoy_feature.language = ip.language and icomparev2.convoy_feature.feature_name = case when ip.type = 'cp' then 'promotion' else 'bv_promotion' end"
print SQLCommand
cursor.execute(SQLCommand)
